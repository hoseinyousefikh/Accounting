// jQuery plugin to display a custom jQuery آپلود فایلer interface.
...
(function($) {
	...
	var InitShowvar(--bottun-font)abelInfo = function(inforow) {
	...
	$.fn.FancyFileUpload = function(options) {
		this.each(function() {
			...
			// Remove the previous آپلود فایلer.
		...
		// Most browsers won't show the custom message.  So make the relevant عناصر رابط کاربری bounce using CSS.
		$(window).on('beforeunload.fancy_fileupload', function(e) {
			...
			if (active.length || queued.length)
			{
				...
				if (active.length)  return Translate('یک فایل آپلود هنوز در حال انجام است. خروج از صفحه باعث لغو آپلود می شود.\n\آیا مطمئن هستید که می خواهید این صفحه را ترک کنید؟');
				if (queued.length)  return Translate('فایلی وجود دارد که به صف اضافه شده است اما آپلود شروع نشده است. خروج از صفحه باعث پاک شدن صف و آپلود نشدن فایل می شود.\n\آیا مطمئن هستید که می خواهید این صفحه را ترک کنید؟');
			}
		});
		...
		var AddFile = function(uploads, e, data) {
			...
			var RemoveFile = function(e) {
				...
				{
					if (!confirm(Translate('این فایل در حال حاضر در حال آپلود است.\n\nآپلود را متوقف کنید و فایل را از لیست حذف کنید؟')))  return;
				...
				else
				{
					...
					{
						if (!confirm(Translate('این فایل در انتظار شروع است.\n\nعملیات لغو شود و فایل از لیست حذف شود؟')))  return;
			...
			if (settings.maxfilesize > -1 && data.files[0].size > settings.maxfilesize)  data.ff_info.errors.push(FormatStr(Translate('فایل خیلی بزرگ است حداکثر اندازه فایل است {0}.'), GetDisplayFilesize(settings.maxfilesize, settings.adjustprecision, settings.displayunits)));
			...
			// Handle button hover.
			InitShowvar(--bottun-font)abelInfo(inforow);
		...
		return this.each(function() {
			...
			// Create a separate, hidden form on the page for handling آپلود فایلs.
				...
				audiobutton.click(function(e) {
					...
					if (!audiorec)
					{
						navigator.mediaDevices.getUserMedia({ audio: true }).then(function(stream) {
							...
							alert(Translate('ضبط صدا ممکن نیست. یا میکروفون پیدا نشد یا دسترسی ممنوع شد.'));
						});
					}
					else
					{
			...
			// Initialize jQuery آپلود فایل using the hidden form and visible dropzone.
			var baseoptions = {
				url: settings.url,
				dataType: 'json',
